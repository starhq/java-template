...
class AuditLogListenerTest {
    ...
    @Test
    void handleEvictEvent_Fail_AuditLogIsNull() {
        // Given: 传入 null 事件
        AuditLogEvent event = new AuditLogEvent(null);
        // When
        auditLogListener.handleEvictEvent(event);
    ...
    @Test
    void handleEvictEvent_Has_Action_Fail_DbException() {
        // Given: 准备数据，但模拟数据库插入时抛出异常
        SysAuditLog mockLog = new SysAuditLog();
        mockLog.setAction("User Save");
        AuditLogEvent event = new AuditLogEvent(mockLog);
