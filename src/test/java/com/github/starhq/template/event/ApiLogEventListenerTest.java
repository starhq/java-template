...
class ApiLogEventListenerTest {
    ...
    @Test
    void handleEvictEvent_Fail_LogIsNull() {
        // Given: 传入 api log
        ApiLogEvent event = new ApiLogEvent(null);
        // When
        eventListener.handleEvictEvent(event);
    ...
    @Test
    void handleEvictEvent_Has_TraceId_Fail_DbException() {
        // Given: 准备数据，但模拟数据库插入抛出异常
        SysApiLog mockLog = new SysApiLog();
        mockLog.setTraceId("traceId");
        ApiLogEvent event = new ApiLogEvent(mockLog);
