...
public class SecurityContextUtils {
     ...
     * populates the context with an anonymous token for unauthenticated requests to allow authorization     * rules to evaluate safely. Excluding it here prevents the business layer from mistakenly treating     * an anonymous user as an authenticated one.     *     * @return an {@link Optional} containing the authenticated {@link Authentication}, or empty if not logged in
     */
     ...
     * is explicitly marked as authenticated, and that the principal is actually an instance of     * {@link UserDetails} before safely casting it.
    ...
    /**
     * Retrieves the current user's username. Strictly requires the user to be logged in.     *     * @return the username string
     *
    ...
    /**
     * Safely retrieves the current user's username without throwing exceptions.     *     * <p>Use this in scenarios where knowing the user is optional, such as recording non-critical
     * audit logs for public APIs, or rendering personalized UI components for guest users.     *     * @return the username string, or {@code null} if the user is not logged in
     */
    ...
    /**
     * Retrieves the current user's database ID. Strictly requires the user to be logged in.     *     * <p>Use this for core business logic where an authenticated identity is a hard prerequisite
     * (e.g., placing an order, changing a password, deleting personal data).     *     * @return the user's ID as a {@link Long}
     *
    ...
    /**
     * Retrieves the collection of authorities (roles/permissions) granted to the current user.     *     * @return a collection of {@link GrantedAuthority}
     *
     * @throws CustomException with {@link ErrorCode#UNAUTHORIZED} if no valid user context is found
     */
    public Collection<GrantedAuthority> getCurrentAuthorities() {
        return getCurrentUserDetails()
                .map(user -> new ArrayList<GrantedAuthority>(user.getAuthorities()))
                .orElseThrow(() -> new CustomException(ErrorCode.UNAUTHORIZED, HttpStatus.UNAUTHORIZED));
    }

    /**
     * Checks if the currently logged-in user holds a specific role or permission authority.     *     * <p>This provides a programmatic way to perform authorization checks directly in the code,
     * serving as an alternative to using {@code @PreAuthorize("hasRole('ROLE_ADMIN')")} annotations.
     * Useful when the permission string is dynamically determined at runtime.     *     * @param roleCode the exact string representation of the authority (e.g., "ROLE_ADMIN" or "sys:user:delete")
     *
